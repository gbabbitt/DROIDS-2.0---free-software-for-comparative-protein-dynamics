...
# view disease or ortholog Frame
my $viewFrame = $mw->Frame(	-label => "CHOOSE SYSTEM TO VIEW",
				-relief => "groove",
				-borderwidth => 2
				);
	my $diseaseRadio = $viewFrame->Radiobutton( -text => "human/disease variant comparison",
						-foreground => 'darkgreen',
                              -value=>"disease",
						-variable=>\$view
						);
     my $orthologRadio = $viewFrame->Radiobutton( -text => "human/ortholog comparison",
						-foreground => 'darkgreen',
                              -value=>"ortholog",
						-variable=>\$view
						);
     my $novelRadio = $viewFrame->Radiobutton( -text => "human/novel variant comparison",
						-foreground => 'darkgreen',
                              -value=>"novel",
						-variable=>\$view
						);
...
$diseaseRadio->pack(-anchor=>"w");
$orthologRadio->pack(-anchor=>"w");
$novelRadio->pack(-anchor=>"w");
...
if ($view eq "disease"){$queryID = $refID."mut";}
if ($view eq "ortholog"){$queryID = $refID."_ortholog";}
...
if ($view eq "disease"){$queryID = $refID."mut";}
if ($view eq "ortholog"){$queryID = $refID."_ortholog";}
if ($view eq "novel"){$queryID = $refID."_novel";}
...
if($attr eq "delta" && $view eq "disease") {
	$input_file = "DROIDS$testStrLong"."AVGdisease.txt";
	$relevant_column = 5;
}
if($attr eq "deltaKL" && $view eq "disease") {
	$input_file = "DROIDS$testStrLong"."AVGdisease.txt";
	$relevant_column = 7;
}
if($attr eq "delta" && $view eq "ortholog") {
	$input_file = "DROIDS$testStrLong"."AVGortholog.txt";
	$relevant_column = 5;
}
if($attr eq "deltaKL" && $view eq "ortholog") {
	$input_file = "DROIDS$testStrLong"."AVGortholog.txt";
	$relevant_column = 7;
}
if($attr eq "delta" && $view eq "novel") {
	$input_file = "DROIDS$testStrLong"."AVGnovel.txt";
	$relevant_column = 5;
}
if($attr eq "deltaKL" && $view eq "novel") {
	$input_file = "DROIDS$testStrLong"."AVGnovel.txt";
	$relevant_column = 7;
}
if($attr eq "gdist") {
