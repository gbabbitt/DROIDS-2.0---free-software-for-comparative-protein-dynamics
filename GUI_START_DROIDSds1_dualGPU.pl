...
my $pdbFrame = $mw->Frame();
	...
	my $RfileFrame = $pdbFrame->Frame();
		my $RfileLabel = $RfileFrame->Label(-text=>"pdb ID human reference (e.g. 5uak) : ");
...
my $launch2Button = $mw -> Button(-text => "launch MD run for disease (pmemd.cuda) - may take many hours", 
				-command => \&launch2,
				-background => 'gray45',
				-foreground => 'white'
				); # Creates a launch button
...
my $align2Button = $mw -> Button(-text => "create sequence and structural alignment for human/disease pair (UCSF Chimera)", 
				-command => \&align2
				); # Creates a align button
...
my $doneButton = $mw -> Button(-text => "parse / prepare human + ortholog files for DROIDS", 
				-command => \&done
				); # Creates a file button
my $done2Button = $mw -> Button(-text => "parse / prepare human + disease files for DROIDS", 
				-command => \&done2
				); # Creates a file button
...
### make disease protein control file ###	
open(my $ctlFile3, '>', "MDd.ctl") or die "Could not open output file";
print $ctlFile3 
"PDB_ID\t".$fileIDr."mutREDUCED\t# Protein Data Bank ID for MD run
Force_Field\t$forceID\t# AMBER force field to use in MD runs
Number_Runs\t$runsID\t# number of repeated samples of MD runs
Heating_Time\t$cutoffValueHeatFS\t# length of heating run (fs)
Equilibration_Time\t$cutoffValueEqFS\t# length of equilibration run (fs)
...
### make novel variant protein control file ###	
open(my $ctlFile4, '>', "MDn.ctl") or die "Could not open output file";
print $ctlFile4 
"PDB_ID\t".$fileIDr."_novelREDUCED\t# Protein Data Bank ID for MD run
Force_Field\t$forceID\t# AMBER force field to use in MD runs
Number_Runs\t$runsID\t# number of repeated samples of MD runs
Heating_Time\t$cutoffValueHeatFS\t# length of heating run (fs)
Equilibration_Time\t$cutoffValueEqFS\t# length of equilibration run (fs)
...
print "MD control files are made (see MDq.ctl, MDr.ctl, MDd.ctl and MDn.ctl)\n";

###################################################
########## make control files for DROIDS###########
###################################################
sleep(0.5);
...
print CTL "query\t"."$fileIDr"."mut\t # Protein Data Bank ID for query structure\n";
print CTL "reference\t"."$fileIDr\t # Protein Data Bank ID for reference structure (or neutral model)\n";
print CTL "length\t"."$lengthID\t # number of amino acids on chain\n";
print CTL "start\t"."$startN\t # number of AA at start of chain\n";
#print CTL "cutoff_value\t"."$cutoffValue\t # p-value under which the KS comparison will be considered significant\n";
...
#print CTL "color_scheme\t"."$colorType\t # output color scheme (red-green, yellow-blue, or orange-magenta)\n";
close CTL;
print("DROIDS ctl file for disease variant is made\n");
sleep(1);
...
print CTL "query\t"."$fileIDr"."_novel\t # Protein Data Bank ID for query structure\n";
print CTL "reference\t"."$fileIDr\t # Protein Data Bank ID for reference structure (or neutral model)\n";
print CTL "length\t"."$lengthID\t # number of amino acids on chain\n";
print CTL "start\t"."$startN\t # number of AA at start of chain\n";
#print CTL "cutoff_value\t"."$cutoffValue\t # p-value under which the KS comparison will be considered significant\n";
...
#print CTL "color_scheme\t"."$colorType\t # output color scheme (red-green, yellow-blue, or orange-magenta)\n";
close CTL;
print("DROIDS ctl file for novel variant is made\n");
sleep(1);
...
open(ctlFile3, '>', "atominfo_$fileIDr"."mut_0.ctl") or die "Could not open output file";
my $parm_label3 = '';
if ($implicit == 1) {my $parm_label3 = "vac_"."$fileIDr"."mutREDUCED.prmtop"; print ctlFile3 "parm $parm_label3\n";}
if ($explicit == 1) {my $parm_label3 = "wat_"."$fileIDr"."mutREDUCED.prmtop"; print ctlFile3 "parm $parm_label3\n";}
my $traj_label3 = "prod_"."$fileIDr"."mutREDUCED_0".".nc";
print ctlFile3 "trajin $traj_label3\n";
print ctlFile3 "resinfo !(:WAT)\n"; # all residues but not water
print ctlFile3 "atominfo \@CA,C,O,N,H&!(:WAT)\n"; # mask for all protein backbone atoms eliminating water
close ctlFile3;

open(ctlFile4, '>', "atominfo_$fileIDr"."_novel_0.ctl") or die "Could not open output file";
my $parm_label4 = '';
if ($implicit == 1) {my $parm_label4 = "vac_"."$fileIDr"."_novelREDUCED.prmtop"; print ctlFile4 "parm $parm_label4\n";}
if ($explicit == 1) {my $parm_label4 = "wat_"."$fileIDr"."_novelREDUCED.prmtop"; print ctlFile4 "parm $parm_label4\n";}
my $traj_label4 = "prod_"."$fileIDr"."_novelREDUCED_0".".nc";
print ctlFile4 "trajin $traj_label4\n";
print ctlFile4 "resinfo !(:WAT)\n"; # all residues but not water
print ctlFile4 "atominfo \@CA,C,O,N,H&!(:WAT)\n"; # mask for all protein backbone atoms eliminating water
close ctlFile4;
...
#######################################
# reset query to novel
open(IN, "<"."DROIDSnovel.ctl") or die "could not find DROIDS.txt file\n";
my @IN = <IN>;
for (my $i = 0; $i < scalar @IN; $i++){
	 my $INrow = $IN[$i];
	 my @INrow = split (/\s+/, $INrow);
...
close MUT;
print "opening mutate_list_diesease.txt using gedit\n\n";
...
system "gedit mutate_list_disease.txt\n";
...
sub orthomutlist{
print "collecting AA substitutions from ortholog alignment file\n";
sleep(1);
	 ...
	 my $header = $INrow[0];
      ...
      if ($header =~ m/$target/){
          ...
          for (my $i = 0; $i <= length $humanseq; $i++){
               $humanAA = substr($humanseq, $i, 1);
               $orthoAA = substr($orthoseq, $i, 1);
               open(IN2, "<"."amino1to3.txt") or die "could not find amino1to3 file\n";
               my @IN2 = <IN2>;
               my $orthoAA3 = '';
               for (my $ii = 0; $ii <= scalar @IN2; $ii++){
	                ...
	                my @IN2row = split (/\s+/, $IN2row);
                     my $oneAA = $IN2row[0];
                     my $threeAA = $IN2row[1];
                     if ($oneAA eq $orthoAA){$orthoAA3 = $threeAA;}
                    }
               close IN2;
...
system "x-terminal-emulator -e perl MD_proteinDisease.pl\n";
print "\n\n";
print "MD SIMULATIONS ARE COMPLETED WHEN TERMINALS CLOSE\n\n";
}
######################################################################################################
...
print "STEP 2 - Then run Match-Align in UCSF Chimera\n\n";
print "            if satisfied with alignment, save as a clustal file with ref PDB ID\n";
print "            in title (e.g. 5uakmut_align.aln)\n\n";
...
open(IN1, "<"."$fileIDq"."_align.aln") or die "could not open alignment...did you save as (.aln)?\n";
...
open(OUT5, ">"."myGranthamDistances_ortholog.txt") or die "could not open output file\n";
...
open(OUT6, ">"."mySeqSTATS_ortholog.txt") or die "could not open output file\n";
...
mkdir ("atomflux_ortholog") or die "please delete atomflux folder from previous run\n";
open (IN, "<"."DROIDSfluctuation.txt") or die "could not create input file\n";
my @IN = <IN>;
open (OUT2, ">"."DROIDSfluctuationAVGortholog.txt") or die "could not create output file\n";
...
for (my $j = 0; $j < scalar @IN; $j++){ # scan atom type
           ...
           open (OUT, ">"."./atomflux_ortholog/DROIDSfluctuation_$next_pos.txt") or die "could not create output file\n";
           print OUT "sample\t"."pos_ref\t"."res_ref\t"."res_query\t"."atomnumber\t"."atomlabel\t"."flux_ref\t"."flux_query\n";
...
sleep(1);
print "\n\n done preparing human/ortholog files for DROIDS\n\n";
sleep(1);
...
sub done2 {   # prep disease mutation run
...
open(OUT4, ">"."$fileIDq"."_vertalign_query_indexed.aln") or die "could not open output file\n";
print OUT4 "respos\t"."seq_query\n";
open(OUT5, ">"."myGranthamDistances_disease.txt") or die "could not open output file\n";
print OUT5 "respos\t"."distance\n";
open(IN1, "<"."$fileIDr"."_alignREFMAP.aln") or die "could not open alignment...did you save as (.aln)?\n";
my @IN1 = <IN1>;
my $position = 0;
...
print "\n\nAA sequence similarity = "."$AAseqsim"."%\n";
print "avg Grantham Distance = "."$avg_gDIST"."\n";
open(OUT6, ">"."mySeqSTATS_disease.txt") or die "could not open output file\n";
print OUT6 "label\t"."value\n";
print OUT6 "AAmatchFreq\t"."$AAseq_matchfreq\n";
print OUT6 "avgGranthamDist\t"."$avg_gDIST\n";
close OUT6;
...
print "\n\n averaging DROIDSfluctuations by residue\n\n";
mkdir ("atomflux_disease") or die "please delete atomflux folder from previous run\n";
open (IN, "<"."DROIDSfluctuation.txt") or die "could not create input file\n";
my @IN = <IN>;
open (OUT2, ">"."DROIDSfluctuationAVGdisease.txt") or die "could not create output file\n";
print OUT2 "pos_ref\t"."res_ref\t"."res_query\t"."flux_ref_avg\t"."flux_query_avg\t"."delta_flux\t"."abs_delta_flux\t"."KLdivergence\n";
@REFfluxAvg = ();
@QUERYfluxAvg = ();
$KL = 0;
for (my $j = 0; $j < scalar @IN; $j++){ # scan atom type
	         ...
	         my @INnextrow = split(/\s+/, $INnextrow); 
			     my $next_pos = $INnextrow[1];
					 ...
					 if ($homology eq "loose"){
					 if(($j == 1 || $pos_ref ne $next_pos) && $res_query ne "xxx"){  # loose homology = collect all aligned residues  
           open (OUT, ">"."./atomflux_disease/DROIDSfluctuation_$next_pos.txt") or die "could not create output file\n";
           print OUT "sample\t"."pos_ref\t"."res_ref\t"."res_query\t"."atomnumber\t"."atomlabel\t"."flux_ref\t"."flux_query\n";
...
print "\n\n done preparing human/disease files for DROIDS\n\n";
...
system "perl GUI_STATS_DROIDSds1.pl\n";	
}
