...
$mw -> title("AMBER 16 MD run on novel variant"); # Titles the main window
$mw->setPalette("gray");
...
my $stopButton = $mw -> Button(-text => "exit to main stats analysis", 
				-command => \&stop
				); # Creates a file button
...
system "perl teLeap_proteinNovel.pl\n";
my $filecheck1 = "vac_".$fileIDr."_novelREDUCED.prmtop";
my $filecheck2 = "wat_".$fileIDr."_novelREDUCED.inpcrd";
my $size1 = -s $filecheck1;
my $size2 = -s $filecheck2;
print "$size1\t"."$size2\n";
if ($size1 <= 10 || $size2 <= 10){print "teLeap may have failed (double check pdb files for problems)\n";}
...
system "pdb4amber -i $fileIDr"."_novel.pdb -o $fileIDr"."_novelREDUCED.pdb --dry --reduce \n";
...
system "x-terminal-emulator -e perl MD_proteinNovel.pl\n";
print "\n\n";
print "MD SIMULATIONS ARE COMPLETED WHEN TERMINAL IS CLOSED\n\n";
}
...
sleep (1);
system "perl GUI_STATS_DROIDSsp2.pl\n";
}
...
print "            in title (e.g. 5uak_novel_align.aln)\n\n";
...
system("$chimera_path"."chimera $fileIDr"."REDUCED.pdb $fileIDr"."_novelREDUCED.pdb\n");
sleep(0.5);
...
open(OUT5, ">"."myGranthamDistances_novel.txt") or die "could not open output file\n";
...
open(OUT6, ">"."mySeqSTATS_novel.txt") or die "could not open output file\n";
...
mkdir ("atomflux_novel") or die "please delete atomflux folder from previous run\n";
open (IN, "<"."DROIDSfluctuation.txt") or die "could not create input file\n";
my @IN = <IN>;
open (OUT2, ">"."DROIDSfluctuationAVGnovel.txt") or die "could not create output file\n";
...
for (my $j = 0; $j < scalar @IN; $j++){ # scan atom type
           ...
           open (OUT, ">"."./atomflux_novel/DROIDSfluctuation_$next_pos.txt") or die "could not create output file\n";
           print OUT "sample\t"."pos_ref\t"."res_ref\t"."res_query\t"."atomnumber\t"."atomlabel\t"."flux_ref\t"."flux_query\n";
...
print "\n\n done preparing human/novel files for DROIDS\n\n";

##################################################################################################
print "\n\n done running MD...close GUI and run SVM to classify impacts\n\n";
sleep(2);
